...
// yarn has the same config as npm
PACKAGE_MANAGER_CONFIGS.yarn = PACKAGE_MANAGER_CONFIGS.npm;
...
const checkDependenciesHelper = (syncOrAsync, config, callback) => {
    ...
    const {packageJsonName, packageJsonRegex, depsJsonName} =
        PACKAGE_MANAGER_CONFIGS[options.packageManager];
    depsDirName = PACKAGE_MANAGER_CONFIGS[options.packageManager].depsDirName;
    ...
    const checkPackage = pkg => {
        ...
        if (options.checkCustomPackageNames && options.packageManager === 'bower') {
